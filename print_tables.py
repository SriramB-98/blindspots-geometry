...
# for j, model_name in enumerate(['resnet50-linf']): #'resnet50-normal', 'resnet50-linf', 'deit_small_patch16_224-normal', 'deit_small_patch16_224-linf']):
    
#     if typ == 'confs':
#         with open(f'./plots_and_figures/{desc}{model_name}_{hyperparam_str}_{dataset}_all_confs.pkl', 'rb') as fp:
...
target_classes_str = '99-199-299-400-499'
desc = 'target_as_source_'
sep = '&'
math_mode = '$'

model_name_form = ['ResNet-50 (Normal)', 'ResNet-50 (AT)', 'DeiT-S (Normal)', 'DeiT-S (AT)']
for j, model_name in enumerate(['resnet50-normal', 'resnet50-linf', 'deit_small_patch16_224-normal', 'deit_small_patch16_224-linf']):
    prefix = f'./plots_and_figures/{desc}{model_name}_{dataset}_{target_classes_str}'
    if typ == 'confs':
        with open(f'{prefix}_{hyperparam_str}_all_confs.pkl', 'rb') as fp:
            avg_confs = pickle.load(fp)
        means, stds = np.round(avg_confs.mean(axis=0), 2), np.round(avg_confs.std(axis=0), 2)
        means = means[:-1]
        stds = stds[:-1]
    if typ == 'dist':
        ...
        dist_means, dist_stds = all_dists.mean(dim=0), all_dists.std(dim=0)
        dist_means[0], dist_stds[0] = dist_means[0]/np.sqrt(224*224*3), dist_stds[0]/np.sqrt(224*224*3)
        means, stds = [], []
        for i in range(len(dist_means)):
            means.append(np.round(dist_means[i].item(), 3))
            stds.append(np.round(dist_stds[i].item(),3))
